...
환경, 동시성과 병렬 프로그래밍, 컴퓨터 비전, 산업 자동화, 게임 프로그래밍, 모바일 앱 개발을
처음부터 새로 집필한 입문서/전문서 모음입니다. 각 책은 9~25개 장 + 부록으로 구성되어 있으며,
장마다 개념 설명·예제 코드·요약·연습문제를 담고 있습니다.
...
- **[MonoGame으로 배우는 C# 게임 프로그래밍](MonoGame/MonoGame기초/00-목차.md)** — XNA의 정신을 계승한 C#/.NET 기반 크로스플랫폼 게임 프레임워크 MonoGame으로 Game 클래스 수명주기, 콘텐츠 파이프라인, SpriteBatch, 입력 처리, 충돌 감지, 오디오, 카메라, 3D 기초, 게임 상태 관리, 실전 미니 게임 제작까지 다룹니다.

## 모바일 앱 개발

- **[Flutter로 배우는 크로스플랫폼 앱 개발](Flutter/Flutter기초/00-목차.md)** — Dart 기반 크로스플랫폼 UI 툴킷 Flutter로 위젯 트리 개념부터 레이아웃, 상태 관리, 네비게이션, 테마, 애니메이션, Form, 비동기 프로그래밍, 네트워크 통신, 로컬 데이터 저장, 플랫폼별 배포, 실전 할 일 관리 앱 제작까지 다룹니다. (이 저장소의 Dart 기초 책을 먼저 읽었다고 가정합니다.)
