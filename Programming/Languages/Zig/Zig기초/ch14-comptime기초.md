...
1장에서 Zig는 C의 전처리기(`#define`, `#ifdef`)나 C++의 템플릿 같은 별도의 매크로 언어 없이도 강력한 메타프로그래밍이 가능하다고 예고했습니다. 그 예고를 위해 1장은 `comptime square(3)`으로 값을 미리 계산하는 예제와, `maxOf(comptime T: type, a: T, b: T) T`로 여러 타입에 동작하는 함수를 만드는 예제를 아주 짧게 맛보기만 했습니다. 이번 장은 그 약속을 완전히 지키는 자리입니다. comptime은 새로운 문법이 아니라, 지금까지 배운 것과 똑같은 Zig 코드를 컴파일이 끝나기 전에 실행해 버리는 Zig 컴파일러의 능력입니다. 이번 장을 마치면 1장의 `maxOf`와 7장에서 잠깐 등장했던 `max(comptime T: type, a: T, b: T) T`라는 시그니처가 정확히 무슨 뜻이었는지 완전히 이해하게 될 것입니다.
