...
```text
             델리게이트 변수가 메서드를 가리키는 구조

1. 델리게이트 변수는 메서드를 가리킨다

+-------------------------+        +--------------------------------+
|  Action<string> greet    | -----> |  void PrintGreeting(string name) |
|  (델리게이트 변수)         |        |  (실제 메서드 본문)                |
+-------------------------+        +--------------------------------+

greet = PrintGreeting;  // 변수에 메서드를 대입
greet("철수");          // 변수를 호출하면 PrintGreeting("철수")가 실행됨

- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

2. event 델리게이트는 여러 구독자를 동시에 가리킬 수 있다

                                    +-------------------------------------+
                              /---> | LogClick(object, EventArgs)          |
+-------------------------+ /      +-------------------------------------+
|  event ButtonClicked     |
|  (발행자가 소유한 이벤트)   |------> +-------------------------------------+
+-------------------------+ \      | ShowMessage(object, EventArgs)       |
                              \     +-------------------------------------+
                               \
                                \-> +-------------------------------------+
                                    | UpdateCounter(object, EventArgs)     |
                                    +-------------------------------------+

구독자(subscriber) 메서드들: 이벤트가 발생하면 등록된 메서드가 차례로 모두 호출된다
```
