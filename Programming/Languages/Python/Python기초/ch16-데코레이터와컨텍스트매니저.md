...
```text
              데코레이터가 함수를 감싸는 구조
   @my_decorator 를 적용하면 원본 함수는 wrapper 함수 안에 감싸인다
                ...
                +---------------------------+
                              ...
                              v
  +------------------------------------------------------------------+
  | wrapper() -- 데코레이터가 반환한 새 함수 (실제로 say_hello 이름표를 |
  | 달고 있음)                                                          |
  |                                                                    |
  |  +--------------------------------------------------------------+ |
  |  | (1) 원본 함수 실행 전 코드 (예: 시작 시각 기록)                   | |
  |  +--------------------------------------------------------------+ |
  |                              |                                    |
  |                              | func(*args, **kwargs) 호출          |
  |                              v                                    |
  |  +--------------------------------------------------------------+ |
  |  | func() -- 원본 함수 (say_hello의 진짜 몸체)                     | |
  |  |    본래 작성했던 로직을 그대로 수행                              | |
  |  |    return 결과값                                               | |
  |  +--------------------------------------------------------------+ |
  |                              |                                    |
  ...
  |                              v                                    |
  |  +--------------------------------------------------------------+ |
  |  | (2) 원본 함수 실행 후 코드 (예: 종료 시각 기록, 로그 출력)         | |
  |  +--------------------------------------------------------------+ |
  |                              |                                    |
  |                              v                                    |
  |                  +---------------------------+                   |
  ...
  +------------------------------------------------------------------+
   ...
   호출자 입장에서는 여전히 say_hello() 하나를 부른 것처럼 보인다
```
