...
```text
              Kotlin 타입 시스템: 널러블 타입과 널이 될 수 없는 타입

+-------------------------------------+   +-------------------------------------+
|              String                 |   |              String?                |
|        널이 될 수 없는 타입          |   |            널러블 타입              |
|                                      |   |                                      |
|  val name: String = "지수"          |   |  val name: String? = null           |
|  name.length -> 항상 안전하게 호출  |   |  name.length -> 컴파일 오류!        |
|  null 대입 시 컴파일 오류 발생      |   |  null 대입 가능                     |
|                                      |   |                                      |
|      [ 컴파일러가 허용 ]            |   |  [ 직접 접근은 컴파일러가 차단 ]    |
+-------------------------------------+   +-------------------------------------+
                                                    |   |   |
                          +-------------------------+   |   +-------------------------+
                          |                             |                             |
                          v                             v                             v
              String? 값을 안전하게 다루는 방법 (아래 세 가지)

+----------------------+   +----------------------+   +----------------------+
|   안전 호출 ?.        |   |   엘비스 연산자 ?:    |   |  non-null 단언 !!     |
|----------------------|   |----------------------|   |----------------------|
| name?.length         |   | name?.length ?: 0    |   | name!!.length         |
| name이 null이면      |   | null일 때 사용할     |   | null이 아니라고       |
| 전체 결과가 null     |   | 대체 값을 지정       |   | 강제로 단언           |
| (예외 없이 안전)     |   | (기본값 제공)         |   | (실제 null이면 예외)  |
+----------------------+   +----------------------+   +----------------------+
           |
           v
+---------------------------------------------------------+
:          let과 안전 호출 결합 패턴 (선택적 사용)          :
:  name?.let { println("이름: $it") }                      :
