...
```text
              ...
              객체 생성 |                    | 객체 생성
                     v                    v
     +--------------------------+   +--------------------------+
     | account1 : BankAccount    |   | account2 : BankAccount    |
     |----------------------------|   |----------------------------|
     | owner_name_ = "김민준"      |   | owner_name_ = "이서연"      |
     | balance_ = 11000.0         |   | balance_ = 5000.0          |
     |                            |   |                            |
     |   독립된 메모리 공간           |   |   독립된 메모리 공간           |
     |   (자신만의 데이터 보유)        |   |   (자신만의 데이터 보유)        |
     +--------------------------+   +--------------------------+

   같은 클래스에서 만들어졌지만, 각 객체는 서로 다른 값을 독립적으로 저장한다
```
