...
```text
       String (널이 될 수 없는 타입)         String? (널러블 타입)
      +---------------------------+     +---------------------------+
      | String name = '지수';       |     | String? nickname;         |
      | 허용                       |     | 허용 (초기값은 null)         |
      +---------------------------+     +---------------------------+
                    |                                 |
                    v                                 v
      +---------------------------+     +---------------------------+
      | name = null;               |     | print(nickname.length);   |
      | *** 컴파일 오류! ***         |     | *** 컴파일 오류! ***        |
      |                             |     | (검사 먼저 필요)             |
      +---------------------------+     +---------------------------+
                    |                                 |
                    v                                 v
      +---------------------------+     +---------------------------+
      | print(name.length);        |     | print(nickname?.length);  |
      | 항상 안전, 검사 없이         |     | 허용, null이면 결과도 null   |
      | 접근 가능                   |     +---------------------------+
      +---------------------------+                   |
                    |                                 v
                    v                     +---------------------------+
      +---------------------------+       | print(nickname ?? '없음');|
      | 규칙: null이 끼어들          |       | 허용, 기본값으로 대체        |
      | 틈이 전혀 없다               |       +---------------------------+
      +---------------------------+

  타입 뒤에 ?가 있는지 없는지를 컴파일러가 대입/접근하는 모든 지점에서 검사한다.
  null 검사를 빠뜨린 코드는 실행되기 전, 즉 컴파일 시점에 걸러진다.
  ?., ??, !, late는 모두 오른쪽의 널러블 타입을 안전하게 다루기 위한 도구다.
```
