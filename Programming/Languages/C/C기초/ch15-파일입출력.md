...
```text
                                   ...
                                   v           |
+-----------+     +--------------+   +------------------+     +-----------+
| fopen()   | --> | fp == NULL?  |   | fread/fwrite     | --> | fclose()  |
| 파일 열기  |     | 오류 확인     |   | fgets/fputs      |     | 파일 닫기  |
| FILE* 획득|     +--------------+   | fscanf/fprintf   |     | 버퍼 비움  |
+-----------+       |      |         +------------------+     +-----------+
                     |      |
                  예(실패)  아니오
                     |      |
  ...
  파일 입출력의 기본 흐름: 열기 -> 오류 확인 -> 읽기/쓰기 반복 -> 닫기
  fclose를 호출해야 버퍼에 남은 데이터가 실제로 디스크에 기록됩니다.
```
