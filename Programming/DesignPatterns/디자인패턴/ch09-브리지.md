...
```text
        브리지 패턴: 추상화(Abstraction)와 구현(Implementation)의 분리

     추상화(Abstraction) 축                      구현(Implementation) 축
       "어떤 도형인가"                              "어떻게 그리는가"

     +----------------------+     참조(다리)     +----------------------+
     |   Shape (abstract)   | ----------------> |   <<interface>>       |
     |  - _renderer:IRenderer|                   |      IRenderer       |
     +----------------------+                    +----------------------+
          ^          ^                                  ^          ^
          |          |                                  : (구현)   : (구현)
     +--------+  +-----------+                   +---------------+ +---------------+
     | Circle |  | Rectangle |                   | VectorRenderer| | RasterRenderer|
     +--------+  +-----------+                   +---------------+ +---------------+
          :
     +-------------------+                              :
     |Triangle (향후 추가) |                     +----------------------+
  ...
  [추상화 쪽 확장]                          [구현 쪽 확장]
  도형 종류가 늘어나도                       렌더링 방식이 늘어나도
  IRenderer 쪽 클래스는                      Shape 쪽 클래스는
  전혀 바뀌지 않는다.                         전혀 바뀌지 않는다.
```
...
솔직히 말하면, .NET BCL 안에서 "이것이 브리지 패턴의 교과서적인 사례다"라고 자신 있게 짚을 만한 뚜렷한 클래스는 마땅치 않다. 위 예제처럼 Abstraction 계층과 Implementation 계층이 각각 독립적인 상속 트리를 이루며 나란히 확장되는 구조를 실무 라이브러리에서 순수한 형태로 찾기는 어렵다.

다만 개념적으로 브리지와 닮은 사례를 하나 들자면 ADO.NET의 공급자(Provider) 모델을 꼽을 수 있다. ADO.NET은 `DbConnection`, `DbCommand` 같은 추상 클래스를 제공하고, SQL Server용 `SqlConnection`, PostgreSQL용 `NpgsqlConnection`처럼 데이터베이스 공급자마다 이 추상 클래스를 구현한 클래스를 따로 둔다. 코드가 구체적인 `SqlConnection` 대신 추상 계층인 `DbConnection`에 의존하도록 작성하면, 나중에 데이터베이스를 SQL Server에서 PostgreSQL로 교체하더라도 연결을 만들고 명령을 실행하는 코드 대부분은 손대지 않고 그대로 동작한다.

```csharp
// DbConnection(추상화 계층)에만 의존하므로 구현체를 바꿔도 이 메서드는 그대로 쓸 수 있다
public decimal GetTotalSales(DbConnection connection)
{
    connection.Open();
    using DbCommand command = connection.CreateCommand();
    command.CommandText = "SELECT SUM(Amount) FROM Sales";
    return (decimal)command.ExecuteScalar();
}

// 호출하는 쪽에서 구현체만 바꿔 끼우면 된다
DbConnection sqlServerConn = new SqlConnection("Server=...");
DbConnection postgresConn = new NpgsqlConnection("Host=...");

GetTotalSales(sqlServerConn);
GetTotalSales(postgresConn); // GetTotalSales 코드는 한 줄도 바뀌지 않는다
```

다만 이 구조를 GoF가 정의한 순수한 브리지와 동일시하기는 어렵다는 점도 짚어야 한다. 브리지 패턴의 핵심은 Abstraction과 Implementation이라는 두 축이 "각자 독립적으로 확장"되도록 의도적으로 설계하는 것인데, ADO.NET의 공급자 모델은 애초에 데이터 접근 계층을 추상화하려는 목적에서 나온 계층 분리의 결과에 가깝다. 즉 결과적으로 "추상화에 의존하면 구현을 갈아 끼울 수 있다"는 브리지와 유사한 이점을 얻지만, 두 계층이 처음부터 상호 독립적인 확장 축으로 설계된 것은 아니라는 차이가 있다.
