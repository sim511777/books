...
```text
              컴포지트 패턴: Leaf와 Composite로 이루어진 트리

                  +--------------------------------------+
                  |          <<interface>> IOrgUnit       |
                  |  GetTotalSalary(), GetHeadCount()     |
                  +--------------------------------------+
                                    : (구현)
                                    v
                         +----------------------+
  ...
  범례
  [ 굵은 테두리 ] Composite (자식을 가진 부서 노드) - 요청을 자식에게 위임
  [ 얇은 테두리 ] Leaf (자식이 없는 개별 직원 노드) - 요청에 자신의 값을 반환
  부모 -> 자식 (모두 IOrgUnit 인터페이스로 동일하게 호출)
```
...
Windows Forms 라이브러리의 `System.Windows.Forms.Control` 클래스는 .NET BCL이 제공하는 대표적인 컴포지트 사례다. `Button`, `TextBox`, `Panel`, `Form` 등 화면에 그려지는 모든 UI 컨트롤은 결국 `Control`을 상속한다. 그런데 `Control`은 동시에 `Controls`라는 프로퍼티(타입은 `Control.ControlCollection`)를 가지고 있어서, 자기 자신의 자식 컨트롤들을 담아 둘 수 있다. `Button`처럼 자식을 두지 않는 컨트롤은 본문에서 말한 Leaf에 해당하고, `Panel`이나 `Form`처럼 다른 컨트롤을 자식으로 담는 컨트롤은 Composite에 해당하는 셈이다.

```csharp
// Leaf: 자식을 갖지 않는 개별 컨트롤
var okButton = new Button { Text = "확인" };
var nameTextBox = new TextBox();

// Composite: 다른 Control들을 Controls 컬렉션에 담을 수 있는 컨트롤
var innerPanel = new Panel();
innerPanel.Controls.Add(nameTextBox);
innerPanel.Controls.Add(okButton);

// Panel 자체도 Control이므로, 더 큰 Composite(Form)의 자식이 될 수 있다
var mainForm = new Form();
mainForm.Controls.Add(innerPanel);

// innerPanel이 낱개 컨트롤인지 여러 컨트롤을 담은 패널인지 구분하지 않고
// 똑같이 Control 타입으로 다룰 수 있다
void HideControl(Control control) => control.Visible = false;
HideControl(okButton);    // 개별 컨트롤도
HideControl(innerPanel);  // 컨트롤 묶음도 동일하게 처리된다
```

여기서 주목할 점은 `HideControl` 같은 메서드가 매개변수로 `Control`을 받기만 하면, 그 인자가 버튼 하나인지 여러 컨트롤을 품은 패널인지 전혀 구분할 필요가 없다는 것이다. `Visible`, `Enabled` 같은 프로퍼티나 `Invalidate()` 같은 메서드를 호출하면, `Panel`이나 `Form`은 내부적으로 자신의 `Controls` 컬렉션을 순회하며 자식들에게 이 요청을 그대로 전파한다. 이는 본문에서 `Department.GetTotalSalary()`가 `_children`을 순회하며 각 자식에게 동일한 연산을 위임하던 방식과 정확히 같은 구조다.

즉 `Control`은 Component 인터페이스 역할과 Composite의 자식 관리 역할을 한 클래스 안에 함께 가지고 있는 형태로, GoF가 제시한 "Leaf와 Composite를 공통 타입으로 다룬다"는 컴포지트의 핵심 아이디어를 실무 UI 프레임워크 수준에서 그대로 보여주는 사례라고 할 수 있다.
