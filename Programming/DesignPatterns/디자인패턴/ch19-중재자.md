...
```text
              중재자 패턴: 그물 구조에서 스타 구조로

  [ 이전: 위젯들이 서로 직접 참조 ]         [ 이후: 모든 위젯이 중재자만 참조 ]
  ...
  +--------------+\ /+-----------+              +--------------+
        ...
        |         / \      |                          v
  +--------------+   +-----------+   +----------+  +-------------------+  +-----------+
  |Agreement     |---|SubmitButton|  |SubmitBtn |->|SignUpFormMediator |<-|EmailField |
  |Checkbox      |   +-----------+   +----------+  |    (Mediator)     |  +-----------+
  +--------------+                                 +-------------------+
                                                            ^
  위젯이 하나 늘어날 때마다                                    |
  ...
  위젯끼리는 서로를 전혀 모르고, 상호작용 규칙은 중재자 하나에 모인다
```
